...
function getLiveGameSetupData() {
  ...
  return {
    ...
    // Recommendations are intentionally loaded in a second request. Building
    // them scans completed games and must never delay or prevent setup opening.
    recommendedPlans: {},
    defaults: {gameType:"Official Game", format:"Quarters", periodLength:8, location:"Home"}
  };
}

function getLiveGameRecommendations() {
  requireStaffCapability_("run_sessions");
  initializeLiveGameSheets_();
  const settings = getCoachIQSettings();
  return getRecommendedLiveGamePlans_(settings.teams || []);
}
...
function getRecommendedLiveGamePlans_(teams) {
  const recommendations = {};
  (teams || []).forEach(function(team) {
    const intelligence = getLiveGameProgramTrends_(team);
    const weak = intelligence.categories.filter(function(item) {
      return item.last3.tracked >= 2 && item.last3.achieved < item.last3.tracked;
    }).slice(0, 3);
    if (!weak.length) return;
    const objectives = weak.map(function(item, index) {
      const lowerBetter = item.categoryKey === "turnovers" || item.categoryKey === "fouls" || item.categoryKey === "featured_player_points";
      const unit = item.categoryKey === "transition_points" || item.categoryKey === "points" || item.categoryKey === "featured_player_points" ? "points" : "count";
      if (item.categoryKey === "featured_player_points") return null;
      return {id:"objective_recommended_" + item.categoryKey + "_" + (index + 1), label:item.label,
        categoryKey:item.categoryKey, subject:"both_teams", goalType:"comparison", unit:unit,
        target:1, direction:lowerBetter ? "lower" : "higher", playerId:"", playerLabel:"",
        active:true, order:index, addedDuringGame:false};
    }).filter(Boolean);
    if (objectives.length) recommendations[team] = {gamesAnalyzed:intelligence.gamesAnalyzed, objectives:objectives,
      reason:"Built from objectives missed in recent completed games."};
  });
