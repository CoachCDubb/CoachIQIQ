...
function recordLiveGameObjectiveEvent(gameId, event) {
  ...
  if (!rows.some(function(row) { return String(row[0] || "") === requestedId; })) {
    const sequence = rows.filter(function(row) { return String(row[1] || "") === String(gameId); }).length + 1;
    const now = new Date();
    eventSheet.appendRow([requestedId || "GEVT-" + Utilities.getUuid().slice(0, 12).toUpperCase(), String(gameId),
      sequence, now, period, "", side, objective.playerId || "", objective.id, delta,
      Session.getActiveUser().getEmail() || "", false, now]);
  }
  updateLiveGameObjectiveState_(gameRecord, period);
  return getLiveGameTracker(gameId);
}
