...
function getScoutingWorkspace() {
  ...
  return {
    ...
    schoolName:settings.schoolName || "", programName:settings.programName || "", season:settings.currentSeason || "",
...
function cleanScoutingSections_(sections) {
  ...
  return sections.map(function(section,index){return {id:safeScoutingText_(section.id,60)||("section-"+index),title:safeScoutingText_(section.title,80)||"Untitled section",prompt:safeScoutingText_(section.prompt,180),content:safeScoutingText_(section.content,5000)};});
}
